...
const MOCK_REPORTS = {
  ...
  tesla: {
    ...
    sources: [
      ...
      { name: "SEC Edgar Database", description: "Federal agency hosting verified annual and quarterly financial filings." },
...
// -------------------------------------------------------------
// CORE FUNCTIONS REQUESTED BY USER
// -------------------------------------------------------------

/**
 * Sends a query to the Context.dev search API
 * @param {string} query 
...
/**
 * Scrapes a single URL to get markdown content
 * @param {string} url 
 * @returns {Promise<string|null>} Markdown text
 */
...
/**
 * Extracts numeric data, years, and facts from Context.dev search results
...
async function extractFacts(searchResults) {
  ...
  // Scrape top 2 URLs for deeper markdown content if they are available
  // This satisfies the "optional image extraction from source metadata/markdown"
  ...
  for (const item of allResults) {
    ...
    // Look for numbers: e.g. 50M, $12B, 1.84M, 150 million, 80% etc.
    // Plus years: e.g. 2020 - 2026
    ...
    // Filter duplicates and extract unique numbers
    ...
    // Extract a descriptive sentence that contains the numbers
    const sentences = textToParse.split(/[.!?\n]/);
    for (const numberStr of uniqueNumbers) {
      // Find a matching sentence for this number to serve as the context/details
...
/**
 * Ranks sources by looking up the URL's credibility
 * Helper function used by rankSources()
 */
function rankSourceDomain(url) {
  ...
  if (lowercaseUrl.includes('sec.gov') || 
    ...
    return 'High'; // 100% official
  }
  
  // Reputable research/news firms
  ...
  // Wikis / medium reliability
  if (lowercaseUrl.includes('wikipedia.org')) {
    return 'Medium';
  }
  
  // Forums / low reliability
...
/**
 * Public wrapper for ranking sources from results
 * @param {Array} results 
...
/**
 * Extracts related images from search results and scraped markdown
 * @param {Array} results 
...
function extractImages(results) {
  ...
  for (const item of results) {
    ...
    // Check if we have a clearbit logo for this domain
    ...
    // Attempt to extract image urls from markdown if present
    if (item.markdown) {
  ...
  // Fallback to cute abstract gradients if we found no images
  if (images.length === 0) {
    images.push(
...
/**
 * Builds the final report answer, handles conflicts, and computes overall confidence score
 * @param {Array} facts 
 * @returns {object} Final synthesized answer meta
 */
function buildFinalAnswer(facts) {
  ...
  // Check if there are numerical values
  ...
  // Look for conflict: variance in values for the same year
  ...
  for (const year of years) {
    ...
    if (yearFacts.length > 1) {
      const values = yearFacts.map(f => {
        // Simple float extraction: e.g. "82.5M" -> 82.5, "$125B" -> 125
      ...
      if (values.length > 1) {
        ...
        // If variance is significant (>25% of min)
  ...
  // Compute confidence score based on source ratings
  // High reliability = 30 points, Medium = 20, Low = 10
  let totalScore = 0;
  facts.forEach(f => {
  ...
  // Penalize confidence if sources conflict
  if (isConflicting) {
  ...
  // Generate dynamic summary paragraphs
...
app.post('/api/research', async (req, res) => {
  ...
  // Route to high-fidelity mock data if the user queries standard template items
  let mockMatch = null;
  ...
  if (mockMatch) {
    // Simulate real delay for loader state verification
    await new Promise(resolve => setTimeout(resolve, 1500));
    return res.json(mockMatch);
  }

  // If CONTEXT_API_KEY is not defined, return a generic fallback matching the layout structure
  ...
  try {
    ...
    // 3. Extract Facts (which scrapes the top results)
    ...
    // Build timeline from facts if multiple years are found
    ...
    return res.json({
      ...
      facts: facts.slice(0, 5), // return top 5 facts
